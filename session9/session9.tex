...
Your tasks today are to choose a decomposition approach, break your product into generic verb--noun functions, build a requirement--function matrix to verify coverage, and search for at least five solution options for each critical sub-function.
\end{tcolorbox}
...
Do not jump straight to technical solutions. Describe functions generically (for example, "move solution"), and avoid decomposing only by physical components.
\end{tcolorbox}
...
Choose the decomposition approach that best suits your product: functional decomposition for interacting subsystems, sequence of user actions for user-centred tasks, or key customer needs for need-driven products. Justify your choice briefly.
\end{tcolorbox}
...
Complete a draft decomposition including the overall function, chosen approach, and at least three major sub-functions in the template. Be ready to explain why you selected this approach.
\end{tcolorbox}
...
\begin{tcolorbox}[title=Hint]
When brainstorming solutions for each sub-function, consider feasibility, alignment with needs, technical maturity, resource requirements, and source credibility. Use both internal brainstorming and external research.
\end{tcolorbox}

Record your findings in the template shown in Table~\ref{tab:session9-solution-template}.
\input{session9/tab_subfunction_solution_template.tex}

\begin{tcolorbox}[title=Checkpoint]
For each critical sub-function, list at least five solution options with sources and feasibility notes. Prepare to merge these into a team table in class.
\end{tcolorbox}
...
\input{session9/planting_flowchart.tex}
% \input{session9/seed_planter_flow.tex}
...
\input{session9/rfm_table_landscape.tex}
