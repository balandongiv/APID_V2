...
Today you will choose the process and structure for your project. Identify a suitable product-development process type, select a flow pattern (generic, spiral, or complex-system), determine which departments must contribute, and propose an organisational structure.
\end{tcolorbox}
...
Do not confuse a development process type (for example market-pull) with a process flow (for example spiral). Remember that departmental structure (who participates) is different from organisational structure (how authority is arranged). Avoid choosing a process without considering product complexity and uncertainty.
\end{tcolorbox}
...
\input{session2/tab_prod_dev_process.tex}
...
Match the process flow to your product's technical uncertainty and subsystem interactions. Use a generic flow for lower-risk projects, a spiral flow when iteration is essential, and a complex-system flow when multiple subsystems must be integrated.
\end{tcolorbox}
...
Each member should prepare notes on the most suitable process type, process flow, essential departments, and recommended organisational structure, each with a short justification. Verify all four decisions are addressed before class.
\end{tcolorbox}
