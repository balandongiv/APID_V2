...
The objectives today are to prepare survey or interview prompts, translate customer statements into concise needs statements, consider whether lead or extreme users should be consulted, and build a structured needs table for your product.
\end{tcolorbox}
...
Frame questions around typical use, likes, dislikes, and suggested improvements. Ensure questions are clear, concise, and unbiased, and justify why each question is useful to your project.
\end{tcolorbox}
...
Before class, each member should produce at least three survey questions with corresponding justifications. Make sure prompts cover monitoring habits, pain points, and improvement suggestions.
\end{tcolorbox}
...
Do not rewrite customer statements as solutions (for example, "use a float sensor"). Needs statements should describe what the product must enable or improve. Avoid assumptions or biases, and keep statements concise and specific.
\end{tcolorbox}
...
\begin{tcolorbox}[title=Hint]
Reflect on whether interviewing lead users or extreme users will reveal hidden needs and edge cases that typical users may not report.
\end{tcolorbox}
...
\input{session7/tab_hydroponic_user_needs.tex}

Lecturer only
\input{session7/tab_customer_needs_thermostat.tex}
...
\begin{tcolorbox}[title=Checkpoint]
Your group table must include agreed prompts, customer statements organised by themes, interpreted needs statements, and variable type. This table is the foundation for specifications in Session 8.
\end{tcolorbox}
